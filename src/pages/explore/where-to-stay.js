...
const IndexPage = (props, {data}) => (
  <Layout>
    <SEO title="Where To Stay In The Cook Islands" />
    <h1 className="text-center">Where To Stay In The Cook Islands</h1>
...
export const pageQuery = graphql`
  query {
    ...
    latestEpisodes: allMarkdownRemark(
      ...
      filter: {frontmatter: {categorySlug: {eq: "where-to-stay/"}}
      }) {
      edges {
        node {
          id
    ...
    allStrapiPodcast(filter: {category: {name: {eq: "where-to-stay"}}}) {
      nodes {
        id
        strapiId
        slug
